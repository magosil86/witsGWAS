...
class PBS_Script(Runnable_Script):
    ...
    def __str__(self):
        ...
        if self.queue == 'WitsLong':
            script.append('#PBS -q WitsLong')
            script.append('#PBS -l nodes=1:ppn=8')
        else:
        ...
        if self.memInGB:
            if self.queue in ['medium', 'WitsLong']:
    ...
    def launch(self):
        cmd="""
        #PBS -N MergeKGSAHGP
        #PBS -l nodes=1:ppn=3,walltime=100:00:00,mem=1GB 
        #PBS -q WitsLong
